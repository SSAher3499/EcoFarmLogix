...
const checkTeamPermission = (action) => {
  return async (req, res, next) => {
    try {
      ...
      if (farm) {
        // Farm owner can do everything with team
        return next();
      }
      ...
      // MANAGER can only view, not invite/remove
      if (action === "view" && ["MANAGER"].includes(farmUser.role)) {
        return next();
      }
      ...
      return res
        .status(500)
        .json({ status: "error", message: "Permission check failed" });
    }
  };
};
